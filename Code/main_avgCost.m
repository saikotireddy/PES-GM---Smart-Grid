...
constant_time = 1;
constant_state = 4 + min_state +1;
constant_price = 1;

constant_action_set = (0:4) + max_askable+1;
...
for main_iter = 1:no_of_iterations
 ...
 %   if main_iter < no_of_iterations - 100
    if rand() < 0.5 && main_iter < no_of_iterations - 100
...
%     end
    ...
    Q(time,state1,state2,max_index) = (1-alpha)*Q(time,state1,state2,max_index) + alpha*(reward + next_max_reward - max(Q(constant_time,constant_state,constant_price,constant_action_set)));
    ...
    if rem(main_iter,100) == 0
        ...
        demand = 2; %demand_values (randi(3));
        battery = 4;%randi(4);
        ren = 2;%randi(4);
        time = 1;%randi(4);
        price = 5;%price_vector(randi(3));
