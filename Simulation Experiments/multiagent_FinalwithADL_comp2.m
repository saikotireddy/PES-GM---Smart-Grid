...
for main_iter = 1:no_of_iterations
     ...
     [~,next_price] = get_next(demand(1),price);
    next_time = time +1;
...
%     end
    ...
    [next_demand,~] = get_next(demand(mini_iter),price);
...
%     
    if rem(main_iter,100) == 0
        ...
        demand(mini_iter) = 4; %demand_values (randi(3));
        battery(mini_iter) = 4;%randi(4);
        ren(mini_iter) = 2;%randi(4);
...
time = next_time;
 if rem(main_iter,100) == 0
     price = price_vector(3);
     time = 2;
 end
 
%fprintf('\n ****************\n');

save('multi_agent2.mat');
