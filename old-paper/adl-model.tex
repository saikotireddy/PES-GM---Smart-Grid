...
We consider $N$ microgrids denoted by $\{1,\ldots,N\}$, which are inter-connected through the distribution network. In this paper, we consider the case when microgrids are connected  to the main grid (i.e, they operate in the grid connected mode). Each microgrid comprises of the distributed small scale renewable power generating sources and are equipped with energy storage devices. Let $B_i$ be the maximum energy storage capacity of microgrid $i$. At every time step $t$ of a day, the $i$th microgrid controller $C_i$ has access to the following information:
\begin{enumerate}[label=(\alph*)]
\item Total generated energy ($r_t^i$) from all its distributed renewable energy sources.
% (super script $i$ used to refer to microgrid $i$).
\item Accumulated non-ADL demand ($d_t^i$) from each load in the $i$th microgrid. 
\item Set of all ADL jobs ( $J_{t}^{i}$) at microgrid $i$.  $J_{t}^{i}$ has the form $\{\gamma_{1}^{i},\ldots,\gamma_{n}^{i}\}$, where the $j$th ADL job $\gamma_{j}^{i}$, is a tuple consisting of  number of units of energy ($a_{j}^{i}$) required to finish that job  and  number of future time slots ($f_{j}^{i}$) remaining by which one can schedule that job without incurring penalty. Let  it be represented as  $\gamma_{j}^{i} = (a_{j}^{i}, f_{j}^{i})$. Let the total ADL demand be denoted by $A_t^i= \sum_{j=1}^{n} a_j^i$.
\item  Total energy available ($b_{t}^{i}$) in the storage device of microgrid $i$.
\end{enumerate} 
...
\end{align}
where the net demand $nd_{t}^{i} = r_{t}^{i} + b_{t}^{i} - d_{t}^{i}$, signifies excess power or deficit depending on whether $nd_{t}^{i} > 0$ or $nd_{t}^{i} < 0$.  Also $p_{t}$ and  $J_{t}^{i}$ denote the price per unit energy and set of all ADL jobs at time instant $t$ respectively. If the net demand $nd_{t}^{i} > 0$  then  there is excess of power after meeting the non-ADL demand and if $nd_{t}^{i} < 0$, there is a deficit in power even to meet the non-ADL demand. The state also includes time since optimal action can depend on it. For example, a microgrid operating on solar renewable generation can sell excess power during the morning as the solar power will be available even during afternoon. But it may not be a good choice to sell it in the evening as there will be no solar power during the night. 
...
Let $P_{t}^{i}$ be the power set of $J_{t}^{i}$, which consists of all possible combinations of the ADL jobs that can be scheduled at time instant $t$ at microgrid $i$. Let  $A_{t}^{i}$ consist of the total aggregated demand  for every subset in  $P_{t}^{i}$. For example, let the $j$th element of $A_{t}^{i}(j) = \sum_{k=1, \gamma_k^i \in P_{t}^{i}(j) }^n a_k^i$, where $ P_{t}^{i}(j)$ is the $j$th element of  $P_{t}^{i}$.
...
\end{align}
where $M_t^i$ denotes the maximum amount of power the main grid can give to microgrid $i$. This constraint is to maintain stability of the main grid. The above bounds indicates that the  microgrids can sell  power only after meeting their non-ADL demand and can buy to fill their battery after meeting their both ADL and non-ADL demands.
...
After the controller picks action $u_{t}^{i}$, we construct the feasible set $F_{t}^{i}$ (subset of $P_{t}^{i}$) which  consists of all possible subsets of ADL jobs that can be scheduled with $u_{t}^{i}$. More formally, each element $j$ of  $F_{t}^{i}$ has to satisfy the following condition:   $A_t^i(j) \leq u_{t}^{i} $, where $A_t^i(j)$ is the total energy required to finish all the ADL jobs in it. The controller picks action $v_{t}^{i}$ which is an element of $F_{t}^{i}$, which results in scheduling all the ADL jobs in that subset. The remaining power is used to meet the non-ADL demand and for storing in the battery for future use.
...
We have $J_{t+1}^{i} = \widehat J_{t+1}^{i} \cup \widetilde J_{t}^{i}$, where $\widetilde J_{t}^{i} =  \{(a_{1}^{i}, f_{1}^{i} - 1),\ldots,(a_{n}^{i}, f_{n}^{i} - 1)\}$, and $ (a_{j}^{i}, y_{j}^{i}) \in \overline J_{t}^{i}$. Further, $\overline J_{t}^{i} = J_{t}^{i} - v_{t}^{i}$.

The storage device battery information is updated as follows:
\begin{align}
b_{t+1}^{i} = max(0,nd_{t}^{i} - u_{t}^{i}),
\end{align}
which denotes the power available after meeting the non-ADL demand while power refused for ADL demand is stored in the battery for future use.
