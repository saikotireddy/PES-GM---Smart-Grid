...
%\author{\IEEEauthorblockN{D. Raghuram Bharadwaj}
...
%\and
%\IEEEauthorblockN{Krishnasuri Narayanam}
%\IEEEauthorblockA{IBM Research - India}
%\and
...
\section{Conclusion}
Providing a unified solution framework for modeling both demand-side management problem (scheduling ADL jobs and optimal utilization of storage devices) and supply-side management problem (enabling cooperative energy exchange among the microgrids) is a challenging task particularly when both demand and supply are considered stochastic. We for the first time in the literature used MDP to integrate these problems into a single unified framework. Also, for the first time in the literature we are considering scheduling ADL demand at micrgrid level as a load shifting technique. RL algorithms provide interesting solution methodology for solving MDP when underlying model is not available. We  used  Q-learning algorithm to maximize profit earned by microgrids by selling excess energy while maintaining demand and supply gap low. Based on the simulation experiments, our algorithms consistently outperforms all other algorithms.

As a future work, we would like to consider the pricing mechanism for micrgrids. The pricing mechanism must allow micrgrids to bid for the selling price and buying price. One can use RL agents to bid for adoptive prices in such a way that micrgrids maximizes their profits. Another important future work is to use efficient RL algorithms with function approximation technique to scale the proposed algorithms. One can use several efficient  feature selection methods to improve the performance of these algorithms.
