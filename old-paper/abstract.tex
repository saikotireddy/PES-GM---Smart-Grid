...
On the supply-side, we study the problem of energy sharing among the microgrids with the  goal of maximizing the profit by selling the excess energy while maintaining the demand and supply deficit low . Also on the demand-side, for the first time , we consider the problem of optimally scheduling the time adjustable smart home appliances demand. In this work, we considered both demand and energy generation as stochastic.
We formulated both the problems in a unified Markov decision process (MDP) framework for the first time to incorporate envisioned future smartgrid system.  We considered the infinite horizon average cost as objective function and used Q-learning algorithm at each microgrid. In simulations,
